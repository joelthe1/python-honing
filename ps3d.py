...
def subStringMatchExactlyOneSub(target,key):
    ...
    exactOccurences = subStringMatchExact(target,key)
    print 'vals are',oneSub, exactOccurences
    for x in range(0,len(oneSub)):
        if oneSub[x] not in exactOccurences:
            exactOneSub += (oneSub[x],)
    return exactOneSub

def callExactlyOneSub(target,key):
    vals = subStringMatchExactlyOneSub(target,key)
    print 'The instances of exactly one substitution is/are:',vals
